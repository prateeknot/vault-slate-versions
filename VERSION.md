...
|---------|---------|-----------|
| **v12.1** | P2 (current) | **IBAN in bottom nav + linked card details** — (1) **IBAN tab added to the bottom nav** (Home / Cards / IBAN / Settings) in the spot Plans used to occupy. (2) **IBAN entries now include linked card details** — migration 0025 adds `card_number`/`expiry`/`cvv` to `iban_cards`; the admin Add/Edit IBAN form has Card Number / Expiry / CVV fields, the admin table shows a Card column, and the user IBAN page shows a "Linked Card" box (masked number + expiry + CVV with copy). AI IBAN tools accept the card fields too. Applied + verified live. |
