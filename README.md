...
- **IBAN Accounts (v12):** admin adds European IBAN accounts in Manage Cards → 🏦 IBAN sub-tab; every user sees ALL active IBANs (latest first, no per-user limit) on the new IBAN page (Cards → "IBAN Accounts" or Settings). Plans moved out of the bottom nav — buy via the center + button, "Add a card" on Cards, or "Upgrade Plan" in Settings.
